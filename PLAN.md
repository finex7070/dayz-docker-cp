...
- Aufruf immer mit Argumentliste, nie `shell=True`:
  ...
  ```
- **`+login` trägt nur den Usernamen.** SteamCMD sagt selbst, welchen Weg es nimmt: `Logging in using cached credentials.` gegen `Logging in using username/password.`. Das Passwort mitzugeben erzwingt jedes Mal den zweiten. Bei Steam Guard per Mail ist das nur Verschwendung; bei **Steam Guard Mobile** muss jeder Credential-Login am Telefon bestätigt werden — aus einer Bestätigung wird eine pro SteamCMD-Lauf, und das Panel hängt an einem Prompt, den der Betreiber nicht sieht (GitHub-Issue von DanielCeregatti). Ohne brauchbaren Token schreibt SteamCMD `Cached credentials not found.` und fragt auf stdin nach dem Passwort; das beantwortet `_answer_prompt`. Erststart und abgelaufener Token funktionieren also weiter, und das Passwort steht nicht mehr in der Prozessliste des Containers.
- `HOME`/`STEAM_HOME` zeigt auf `/data/steam` → Sentry-Datei und Session-Token überleben Container-Neustarts
- **Steam-Guard-Handling:** Die Ausgabe wird zeilenweise auf Marker wie `Two-factor code`, `Steam Guard code` bzw. `FAILED (Two-factor code mismatch)` geprüft. Trifft einer zu, geht der Job in Status `needs_guard`, das Panel zeigt ein Eingabefeld, und der Lauf wird mit dem eingegebenen Code wiederholt. `STEAM_GUARD_CODE` aus der Env wird beim **ersten Guard-Prompt** automatisch verwendet — getrennt von den übrigen Prompts gezählt, weil seit dem passwortlosen `+login` ein Passwort-Prompt davor liegen kann.
...
- **Stop:** `SIGTERM` → bis zur eingestellten Frist warten → `SIGKILL`; Statusmaschine mit Lock verhindert parallele Aktionen. Die Frist gehört dem Betreiber (`stop_timeout_seconds`, Standard 30 s, 5–600 s): ein voller Server schreibt seine Persistenz länger als ein leerer. Derselbe Knopf bietet während des Wartens **Kill** an, das sofort `SIGKILL` schickt — der Ausweg aus einem Shutdown, der nicht vorankommt, bezahlt mit dem, was noch nicht geschrieben war. Beim Herunterfahren des Containers gilt stattdessen `CONTAINER_STOP_TIMEOUT` (30 s), siehe §7.5. Die Frist wird jedoch **abgekürzt, sobald die Engine fertig ist**: nach `~DayZGame()` beziehungsweise `--- Termination successfully completed ---` steht die Persistenz auf der Platte, und was danach hängt, hängt endgültig. DayZ stirbt gelegentlich im eigenen Teardown (`double free or corruption (out)`, gemeldet mit Expansion) und beendet sich nie — dafür die vollen 60 s zu warten bringt nichts, also gibt `ENGINE_DONE_GRACE_SECONDS` (10 s) dem Prozess noch eine kurze Frist und die Meldung sagt, dass gespeichert wurde.
